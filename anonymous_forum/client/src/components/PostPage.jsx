...
class PostPage extends Component {
  ...
  checkLike() {
    ...
    //console.log(user);
    // if (post.upvotes && post.upvotes.includes(user._id)) return true;
    // else return false;
    return true;
  }
  ...
  checkReport() {
    ...
    const { post } = this.state;
    //console.log(user);
    // if (post.reports && post.reports.includes(user._id)) return true;
    // else return false;
    return true;
  }
  checkReplyLike(id) {
    ...
    if (user) {
      for (let i in replies) {
        if (replies[i]._id === id) {
          // if (replies[i].upvotes.includes(user._id))
          return true;
        }
      }
    }
  ...
  render() {
    ...
    return (
      <div>
        ...
        {<PostReply id={this.props.match.params.id} />}
        <div className="container col-lg-6 shadow-lg p-3 mt-5 bg-body rounded">
